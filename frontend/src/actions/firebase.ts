...
export const sendSignupLinkViaEmail = async (email: string, targetPage: string, userName?: string, userId?: string) => {
    await sendSignInLinkToEmail(auth, email, { url: process.env.DOMAIN_URL + '/logged-handler?mailLink=true' + (userName ? "&username=" + userName : "") + (userId ? "&userId=" + userId : "") + (targetPage ? "&target=" + targetPage : ""), handleCodeInApp: true })
