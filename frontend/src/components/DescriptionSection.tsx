...
export const Testimonial = ({ data }: { data?: APIResponse<"api::testimonial.testimonial">["data"]["attributes"] }) => {
  ...
  return <div className="text-center">
    ...
    <Card className="flex flex-col md:md:flex-row items-center overflow-hidden">
      ...
      <div>
        <Image src={"http://localhost:1337" + data.Image?.data.attributes.formats?.medium.url} width={data.Image?.data.attributes.formats?.medium.width} height={data.Image?.data.attributes.formats?.medium.height} alt="immagine del testimonial" className="transform md:translate-x-[-15%] translate-y-[15%] scale-[130%]" />
      </div>
    </Card>
  </div>
}
