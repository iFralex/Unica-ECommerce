    ...
    return (
        <div className="relative flex justify-center bg-gradient-to-r from-[#fffffff0] to-black/30 md:to-transparent pt-16 pb-12 px-8">
            ...
            <div className="absolute inset-0 z-[-1] flex justify-end bg-white overflow-x-hidden">
                <Image className="w-full object-right object-cover overflow-x-hidden" src={product.attributes.MainImage?.data.attributes.formats?.large.url} alt="Immagine" width={product.attributes.MainImage?.data.attributes.formats?.large.width} height={product.attributes.MainImage?.data.attributes.formats?.large.height} />
            </div>
        </div>
    )
}
